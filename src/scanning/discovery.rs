...
fn parse_sony_device(response: &str, ip: Ipv4Addr) -> Option<PS4> {
    Some(PS4 {
        ...
        host_request_port: get_header(response, "host-request-port")?.parse().ok()?,
        device_discovery_protocol_version: get_header(
            response,
                ...
                match socket.recv_from(&mut buffer) {
                    Ok((size, sender)) => {
                        ...
                        if let IpAddr::V4(ip) = sender.ip() {
                            if let Some(ps4) = parse_sony_device(&response, ip) {
